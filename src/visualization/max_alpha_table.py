...
grouped_auc = grouped.apply(
    lambda x: integrate(y=extend_with_number(x.pair_completeness, 1), x=extend_with_number(x.reduction_ratio, 0.5))
)
...
def add_parameters(row, parameter_config_list):
    params = next(
        ...
        if (
            abs(x["threshold"] - row["threshold"]) < 0.02
