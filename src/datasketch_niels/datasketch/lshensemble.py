...
import numpy as np
from .storage import _random_name
from .lsh import integrate, MinHashLSH
...
class MinHashLSHEnsemble(object):
    ...
    def __init__(
        self, threshold=0.9, num_perm=128, num_part=16, m=8, weights=(0.5, 0.5), storage_config=None, prepickle=None
    ):
    ...
    def _init_optimal_params(self, weights):
        ...
        self.params = np.array(
            [
                _optimal_param(self.threshold, self.h, self.m, xq, false_positive_weight, false_negative_weight)
                for xq in self.xqs
            ],
            dtype=np.int,
        )
